			...
			<div class="col-sm-16 col-md-16">
				<div class="card">
					<div class="card-footer align-items-center justify-content-between d-flex">
						<button class="btn btn-outline-success pull-right">Client Review / Pleasent List</button>
						<button class="btn btn-primary NewsEventAddButton" ><i class="fa fa-plus"></i> Client Review/Pleasent Add</button>
					</div>
...
<div class="modal dark_bg fade" id="GallaryAdd" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel2" aria-hidden="true">
	<div class="modal-dialog modal-lg" role="document">
		<form id="notice_board_insert_update" action="{{route('client_review.update')}}" accept-charset="utf-8"  enctype="multipart/form-data" method="post"  class="form-horizontal validatable">
					...
					<div class="row">
						<div class="col-md-16 -16  text-white">
							<div class="form-group">
								<label for="example-search-input" class="col-12 col-form-label"> Client Review </label>
								<div class="col-16">
...
<script>
	$(document).ready(function(){
		$('#notice_board_table').DataTable({
			...
			serverSide: true,
			ajax: {
				url: "{{route('client_review.data')}}",
				type: 'GET',
				cache: false
			},
			columns: [
            { title: 'SL', data: 'id', name: 'id' },
            { title: 'Client Review', data: 'image', name: 'image' },
		...
		$(document).on('click','.tableDelete',function () {
			...
			$(this).ajaxSubmit({
				...
				method: 'POST',
				dataType: 'json',
				url: "{{route('client_review.update')}}",
