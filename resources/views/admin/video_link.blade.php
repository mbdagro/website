			...
			<div class="col-sm-16 col-md-16">
				<div class="card">
					<div class="card-footer align-items-center justify-content-between d-flex">
						<button class="btn btn-outline-success pull-right">Video Link List</button>
						<button class="btn btn-primary NewsEventAddButton" ><i class="fa fa-plus"></i> Video Link Add</button>
					</div>
...
<div class="modal dark_bg fade" id="GallaryAdd" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel2" aria-hidden="true">
	<div class="modal-dialog modal-lg" role="document">
		<form id="notice_board_insert_update" action="{{route('video_link.update')}}" accept-charset="utf-8"  enctype="multipart/form-data" method="post"  class="form-horizontal validatable">
					...
					<div class="row">
						<div class="col-md-16 -16  text-white">
							<div class="form-group">
								<label for="example-search-input" class="col-12 col-form-label"> Link </label>
								<div class="col-16">
									<input name="link" type="text" class="form-control" value="" placeholder="Link">
								</div>
							</div>
						</div>
					</div>
...
<script>
	$(document).ready(function(){
		$('#notice_board_table').DataTable({
			...
			serverSide: true,
			ajax: {
				url: "{{route('video_link.data')}}",
				type: 'GET',
				cache: false
			},
			columns: [
            { title: 'SL', data: 'id', name: 'id' },
            { title: 'Link', data: 'video_link', name: 'video_link' },
            { title: 'Added By', data: 'user_id', name: 'user_id' },
		...
		$(document).on('click','.tableEdit',function () {
			...
			$(this).ajaxSubmit({
				...
				dataType: 'json',
				method: 'GET',
				url: "{{route('video_link.edit')}}",
				success: function (responseText) {
					$('input[name^="link"]').val(responseText.data.video_link);
					$("#GallaryAdd").modal('show');
				}
			});
		});
